...
import locatorstrategyform.StateVertexLevensteinEquals;
...
public class UtilsCrawler {
	...
	public static void myCrawlRules(CrawljaxConfigurationBuilder builder) throws Exception {
		...
		builder.crawlRules().dontClick("a").withAttribute("id", "lnkSignIn");
		builder.crawlRules().dontClick("a").withAttribute("id", "lnkForgotPassword");
		...
		if (browser.equals("FF")) {
			System.setProperty("webdriver.gecko.driver","C:\\Users\\hem.bist\\Downloads\\geckodriver-v0.32.0-win-aarch64\\geckodriver.exe");
			builder.setBrowserConfig(new BrowserConfiguration(BrowserType.FIREFOX, 1));
		} else if (browser.equals("PH")) {
			builder.setBrowserConfig(new BrowserConfiguration(BrowserType.PHANTOMJS, 1));
		}else if (browser.equals("CH")) {
			System.setProperty("webdriver.chrome.driver","C:\\NewCode\\Driver\\gem-hem\\drivers\\ChromDriver\\chromedriver_109.exe");
...
//				String submit_text = configFile.getProperty("SUBMIT_TEXT");
				...
				loginForm.field("username").setValue("jasleen.multani");
				loginForm.field("password").setValue("Gemini@123");
				input.setValuesInForm(loginForm).beforeClickElement("name").withText("submit");

				
			}
			
			if (url.contains("orangehrm")) {
//				String user_field = configFile.getProperty("USERNAME_FIELD");
//				String user_value = configFile.getProperty("USERNAME_VALUE");
//				String password_field = configFile.getProperty("PASSWORD_FIELD");
//				String password_value = configFile.getProperty("PASSWORD_VALUE");
...
//				String submit_text = configFile.getProperty("SUBMIT_TEXT");
				...
				loginForm.field("password").setValue("admin123");
				input.setValuesInForm(loginForm).beforeClickElement("type").withText("submit");
