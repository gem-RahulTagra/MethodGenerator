...
public class Clean {
	...
	private static void usage() {
		System.out.println("Usage:  java -cp :./jsoup-1.7.2.jar:./commons-io-2.4.jar:./jtidy-r938.jar"
				+ " Clean -u <url> | -f <file>");
	}
	...
	public static void clean(FileInputStream input, FileOutputStream output) {
		...
		/*
		 * 
		 * // set configuration values tidy.setDropEmptyParas(false); // drop empty P
		 * elements tidy.setTrimEmptyElements(false); tidy.setFixUri(false);
		 * tidy.setFixBackslash(false); tidy.setDocType("omit"); // omit the doctype
		 * tidy.setEncloseBlockText(false); // wrap blocks of text in P elements
		 * tidy.setEncloseText(false); // wrap text right under BODY element in P
		 * elements tidy.setHideEndTags(false); // force optional end tags
		 * tidy.setIndentContent(false); // indent content for easy reading
		 * tidy.setLiteralAttribs(false); // no new lines in attributes
		 * tidy.setLogicalEmphasis(false); // replace i and b by em and strong,
		 * respectively tidy.setMakeClean(false); // strip presentational cruft
		 * tidy.setNumEntities(true); // convert entities to their numeric form
		 * tidy.setWord2000(false); // strip Word 2000 cruft tidy.setXHTML(true); //
		 * output XHTML tidy.setQuoteNbsp(true); tidy.setXmlPi(false); // add <?xml?>
		 * processing instruction tidy.setPrintBodyOnly(false);
		 * tidy.setIndentAttributes(false); tidy.setIndentCdata(false);
		 * tidy.setIndentContent(false); tidy.setBreakBeforeBR(false);
		 * tidy.setSpaces(0); tidy.setHideComments(true); tidy.setFixComments(true);
